...
## 6. 1인 운영 원칙 (Yohan OS 적용)

OpenAI 하네스 엔지니어링 사례([원문](https://openai.com/ko-KR/index/harness-engineering/) / 로컬: `memory/ingest/url/url-5c5e7aedc9912aae.md`)에서 추출한 패턴을 **Yohan 1인 + AI 에이전트** 규모로 번역한 원칙이다. `AGENTS.md` §0과 동일 축.

| # | 원칙 | OpenAI 원본 | 1인 번역 |
|---|------|-----------|---------|
| 1 | 역할 전환 | 엔지니어가 코드 대신 환경·피드백 루프 설계 | 코드보다 **규칙·맥락(`memory/rules/`)** 정비에 시간 투자 |
| 2 | 짧은 진입 문서 | AGENTS.md ~100줄 = 목차, 세부는 `docs/` | `.cursorrules` 150줄 이하 유지, 세부는 `memory/rules/*`·`docs/*`로 분리 |
| 3 | 지식 = repo 아티팩트 | "repo에 없으면 존재하지 않는 것" | 대화 속 결정 → 즉시 `append_decision`. 나중은 없음 |
| 4 | 강한 경계 + 로컬 자율 | 린트·구조 테스트로 레이어 강제, 구현은 자유 | 경계 5개 이하(`must_not`, SoT 우선, Evaluator 게이트 등)만 기계적 강제 |
| 5 | 평가 루프 내장 | 에이전트 간 리뷰 + 사람 선택적 리뷰 | P/G/E + Evaluator `pass`는 빠르게 훑고, `revise`/`reject`만 집중 |
| 6 | 상시 GC | 매일 소규모 정리 PR > 주간 대규모 리팩터 | 세션당 1건 정리 (깨진 링크, 오래된 규칙, 중복 파일 등) |
| 7 | 에이전트 친화 구조 | 파일 크기 제한, 네이밍, 경계 파싱 린트 | 새 도구·포맷 도입 전 "에이전트가 읽고 바로 쓸 수 있나?" 자문 |

### 주의

- OpenAI 사례는 **전용 투자·팀 규모** 기준이므로 그대로 복제하지 않는다.
- 1인은 **작고 안정적인 루프**가 맞다. "전면 자동화"보다 **한 바퀴씩 확실히 도는 것**이 우선.
