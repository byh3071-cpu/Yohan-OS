...
| **하네스 엔지니어링**  | **순서·도구·금지·검증**으로 출력을 신뢰 가능하게 묶음      | `memory/rules/agent-harness.md`, P/G/E(`pge-pipeline.md`), Evaluator 게이트, MCP `plan_task`·`append_decision` 등 |
...
| 종류                | 경로                                                                                                 |
| ----------------- | -------------------------------------------------------------------------------------------------- |
| 컨텍스트 SoT          | `memory/profile.yaml` (`differentiation` 여백 포함), `memory/active-project.yaml`, `memory/decisions/` |
